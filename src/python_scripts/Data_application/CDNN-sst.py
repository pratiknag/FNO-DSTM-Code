...
"""
Created on Friday May 29 2025

@author: Pratik

CDNN Training Script for SST or Precipitation Data
--------------------------------------------------

This script trains a convolutional-deconvolutional neural network (CDNN) on spatio-temporal datasets such as 
Sea Surface Temperature (SST) or Precipitation.
...
def main():
    parser = argparse.ArgumentParser(description='Train CDNN on SST data')
    ...
    if train:
        print("training started --- CDNN training phase")
        for ep in range(epochs):
            ...
            if train_mse < best_val_loss:
                ...
                # Save the model
                torch.save(model.state_dict(), 'models/'+str(dat_name)+'-CDNN.pth')
                ...
                if epochs_without_improvement >= 6:
                    ...
                    break
        model.load_state_dict(torch.load('models/'+str(dat_name)+'-CDNN.pth', weights_only=True))
        # freeze(model)
        # model.eval()
        interval = 3
        best_val_loss = float('inf')
        for ep in range(epochs ):
            ...
            if train_l2 < best_val_loss:
                ...
                epochs_without_improvement = 0
                # Save the model
                torch.save(cov_model.state_dict(), 'models/'+str(dat_name)+'-CDNN-cov.pth')
                torch.save(model.state_dict(), 'models/'+str(dat_name)+'-CDNN.pth')
            else:
                epochs_without_improvement += 1
        ...
        model_path = 'models/'+str(dat_name)+'-CDNN.pth'
    ...
    model.load_state_dict(torch.load('models/'+str(dat_name)+'-CDNN.pth', weights_only=True))
    cov_model.load_state_dict(torch.load('models/'+str(dat_name)+'-CDNN-cov.pth', weights_only=True))
    ...
    pred = pred.numpy()
    cov1 = cov1.numpy()
    np.save("pred/"+str(dat_name)+"-CDNN-pred.npy",pred)
    np.save("pred/"+str(dat_name)+"-CDNN-cov.npy",cov1)
    np.save("pred/"+str(dat_name)+"-CDNN-test.npy", test_u)
